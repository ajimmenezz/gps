  ...
  <div class="col-12" style="margin-top: 50px;">
    <div class="col-12" v-if="!isEmptyReports">
      <h5 class="float-left text-muted" style="font-size: 20px; padding-bottom: 10px;">
        <b>Tiempo encendido y sin movimiento</b>
      </h5>
    </div>
      ...
      >
        ...
        <tbody>
          <tr v-for="(item,index) in  gwtData" :key="index">
            ...
            <td>{{item.to.date}}</td>
            <td>
              <span v-if="item.ralentiTime.days > 0">{{item.ralentiTime.days}} días</span>
              <span v-if="item.ralentiTime.hours > 0">{{item.ralentiTime.hours}} horas</span>
              {{item.ralentiTime.minutes}} minutos
            </td>
          </tr>
        </tbody>
      </table>
