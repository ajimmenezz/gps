<template>
 ...
 <form id="validation-form123" action="" @submit.prevent>
                <div class="row float-left">
                     ...
                     <config-text-typography  texto="Limite de exceso de velocidad *"></config-text-typography>
                        <input type="number" id="vel" class="form-control" placeholder="Limite de exceso de velocidad" min="50" v-model="velocidad" name="validation-velocidad">

                  </div>
                  <div col="col-2" style="margin-top: 45px;"><config-text-typography  texto="km/hr"></config-text-typography></div>
...
export default {
   ...
   */
    async SendForm () {
      ...
      if (this.velocidad != null && this.velocidad != null && this.order != '') {
        if (this.velocidad < 50) {
          $('#alertReportConfig').html("<div class='alert alert-danger alert-dismissible fade show' role='alert' style='text-align: initial;'>La velocidad no puede ser menor a 50 km/hr<button type='button' class='close' data-dismiss='alert' aria-label='Close' style='padding-top: 0px;'><span aria-hidden='true'>&times;</span></button></div>")
          return false
        }
        console.debug('entra')
        this.$store.state.loader = true
