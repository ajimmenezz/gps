...
export default {
   ...
   */
    async getUsers () {
      ...
      console.debug('USUARIOS', this.userList)
    },

    OnUserSelected (dat) {
      console.debug('user', dat)
      if (dat == -1) {
   ...
   */
    async SendForm () {
      ...
      if (this.user != null && this.user != '' && this.order != '') {
        console.debug('entra')
        this.$store.state.loader = true
