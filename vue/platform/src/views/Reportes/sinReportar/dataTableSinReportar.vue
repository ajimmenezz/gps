      ...
      >
        ...
        <tbody>
          <tr v-for="(item,index) in gwtData" :key="index">
            ...
            <td>{{item.to.date}}</td>
            <td>
               <span v-if="item.totalTime.days > 0">{{item.totalTime.days}} días </span>
               <span v-if="item.totalTime.hours > 0">{{item.totalTime.hours}} horas </span>
               {{item.totalTime.minutes}} minutos
            </td>
          </tr>
        </tbody>
      </table>
