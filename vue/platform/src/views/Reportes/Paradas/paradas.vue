    ...
    left: -17px; margin-top: 27px; padding-left: 22px;">
                  <i class=" icon-md universalicon-calendar " style="padding: 4px; padding-top: 5px;"></i>
                   ...
                   <div class="col-10">
                     ...
                     <config-text-typography  texto="Tiempo de unidad parada *"></config-text-typography>
                        <input type="number" id="time" class="form-control" placeholder="Tiempo de unidad parada" min="5" v-model="tiempo" name="validation-tiempo">

                  </div>
                  <div col="col-2" style="margin-top: 45px;"><config-text-typography  texto="minutos"></config-text-typography></div>
...
* @vuese
 * @group reports
 * form para reporte de accesos al sistema
 */
export default {
  name: 'reportAccesos',
   ...
   */
    async SendForm () {
      console.debug('LLAMDA', this.tiempo)

      if (this.unidad != null && this.unidad != '' && this.order != '' && this.tiempo != null && this.tiempo != '') {
        if (this.tiempo < 5) {
          $('#alertReportConfig').html("<div class='alert alert-danger alert-dismissible fade show' role='alert' style='text-align: initial;'>El tiempo no puede ser menor a 5 minutos<button type='button' class='close' data-dismiss='alert' aria-label='Close' style='padding-top: 0px;'><span aria-hidden='true'>&times;</span></button></div>")
          return false
        }
    ...
    async modalSend (filters) {
      ...
      subtittle = 'Permite diseñar desde el inicio la estructura de un reporte y el tipo de datos que va a incluir.'
