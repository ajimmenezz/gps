        ...
        <div class="col-12">
                ...
                <div class="float-left tab-content " id="myTabContent" style=" box-shadow: none; width: 100%;">
                    <!-- <form @submit.prevent > -->
                           ...
                           <div  v-if="!this.$store.state.typeDevice.mobileOrTable" class=" tab-pane fade show active " id="legales"   role="tabpanel" aria-labelledby="legales-tab">
                            <div class="row">
                          ...
                          <div v-if="this.$store.state.typeDevice.mobileOrTable" class=" tab-pane fade show active " id="legales"   role="tabpanel" aria-labelledby="legales-tab">
                            <div class="row">
                              ...
                              <div class="col-12 col-md-6 float-right">
                                <button v-if="!Getdisabled" id="editSubmit" type="submit" class="btn btn-primary" @click="editarCampos(true)" >Editar datos</button>
                                <button v-if="Getdisabled" class="btn btn-outline-primary " type="button" @click="editarCampos(false)">Cancelar actualizacion</button>
                                <button v-if="Getdisabled" id="registerSubmit" type="submit" class="btn btn-primary " @click="SendForm()" >Guardar cambios</button>
                              </div>
...
export default {
   ...
   */
    ...
    OnEdit (item, sender) {
      // alert(`EDIT: ${item.name} ${item.id} ${item.tipo}`)
    ...
    //  this.agregarContacto(item)
    },
    OnDelete (item, sender) {
      alert(`DELETE: ${item.name} ${item.id} ${item.tipo}`)
