<template>
    ...
    <div id="modalAdminProduct" class="modal fade " tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog  modal-lg" role="document">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalCenterTitle" v-if="this.accion=='nuevo'">Nuevo Producto</h5>
                  <h5 class="modal-title" id="exampleModalCenterTitle" v-if="this.accion=='editar'">Editar Producto</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
              </div>
               <form @submit.prevent >
              <div class="modal-body float-left" >

                 <p v-if="this.$store.state.modal.datosDymanic.idCliente != 0" class="text-muted">El producto se agregará en el almacén del cliente: <b>{{this.$store.state.modal.datosDymanic.nameClient}}</b></p>
                <p v-else>El producto se agregará en tú almacén</p>
...
export default {
   ...
   */
    async modelProduct () {
      ...
      var request = await this.getRequest('catalogs/products/factories/' + this.marca + '/models')
